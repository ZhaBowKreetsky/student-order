...
public class StudentOrderDaoImpl implements StudentOrderDao {
    ...
    private static final String SELECT_ORDERS =
            "SELECT so.*, ro.r_office_area_id, ro.r_office_name, " +
                    ...
                    "FROM ps_student_order so " +
                    "INNER JOIN ps_register_office ro ON ro.r_office_id = so.register_office_id " +
                    "INNER JOIN ps_passport_office po_h ON po_h.p_office_id = so.h_passport_office_id " +
                    "INNER JOIN ps_passport_office po_w ON po_w.p_office_id = so.w_passport_office_id " +
                    "WHERE student_order_status = ? ORDER BY student_order_date LIMIT ?";


    private static final String SELECT_CHILD =
            "SELECT soc.*, ro.r_office_area_id, ro.r_office_name " +
                    "FROM ps_student_child soc " +
                    "INNER JOIN ps_register_office ro ON ro.r_office_id = soc.c_register_office_id " +
    ...
    private static final String SELECT_ORDERS_FULL =
            "SELECT so.*, ro.r_office_area_id, ro.r_office_name, " +
                    ...
                    "INNER JOIN ps_passport_office po_h ON po_h.p_office_id = so.h_passport_office_id " +
                    "INNER JOIN ps_passport_office po_w ON po_w.p_office_id = so.w_passport_office_id " +
                    "INNER JOIN ps_student_child soc ON soc.student_order_id = so.student_order_id " +
                    "INNER JOIN ps_register_office ro_c ON ro_c.r_office_id = soc.c_register_office_id " +
